...
public interface PostCostRepository extends JpaRepository<PostCost, PostCostId>{
	...
	@Query(value="SELECT pc "
		...
		+ "AND :startDate BETWEEN pc.postCostId.startDate AND pc.postCostId.endDate "
		+ "AND :endDate BETWEEN pc.postCostId.startDate AND pc.postCostId.endDate "
		+ "AND :startDate < pc.postCostId.endDate "
		)
	List<PostCost> findActivePostCostByPostIdAndDates(@Param("postId")long postId, @Param("startDate") Date startDate, @Param("endDate") Date endDate );
